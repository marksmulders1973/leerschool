...
### 📏 Welke telling is het hoofdcijfer (Mark 25 sep 2026: "sep 1347, klopt dat exact?" → "ja doe alle drie")

- **Hoofdcijfer = unieke apparaten uit `events_mens`** (= `events_echt` mín mailscanner-golven: view `scanner_uids`, ≥8 apparaten binnen ±3 min met ≤4 "openen"-events in ≤30 s). Nooit meer ruwe `events` of `events_echt` voor tellingen van mensen; sommen/kwartier mogen via beide (scanners maken geen sommen).
- **Hosting-dashboard (Vercel "visitors") alleen als vergelijking eronder**, altijd mét periode ("t/m 23 sep") en de kanttekening dat Vercel per dag telt (dezelfde persoon op 5 dagen ≈ 5 bezoekers) → geen aantal mensen.
- In de jaarlijn: groene staaf = apparaten (events_mens); hosting-cijfer als klein grijs getal/lijntje.
- Nulmeting 25 sep: sep 1.110 → **1.066** mens · aug 472 → **377** · do 24 sep 130 → **98**. 139 scanner-apparaten totaal (bulkmail-momenten 9 aug, 18 aug, 31 aug, 1 sep, 24 sep).
...
- **Deel B — Route naar de top (langzaam bewegend, elke sessie licht bijwerken):** (1) *Waar we nú aan werken* (lopend werk + ideeën-tracker-status); (2) **💡 Ideeën-generator** — de uitgewerkte concepten mét effort-tags **plus een vers idee alléén wanneer er écht iets waardevols is** (Mark 2 aug: "indien nodig" — niet forceren; een sessie zonder nieuw idee is prima, liever niets dan opvulling); (3) 🔭 Concurrentie-wig (vergelijkingstabel + positioneringszin + lessen); (4) 🚀 Route naar de top (groei-motoren op hefboom + warme leads/outreach); (5) **💶 Subsidie-spoor** (SIDN-pionier → Stichting Leerkwartier + ANBI → overheid-consortium; bron `docs/SUBSIDIE-PLAN.md`); (6) **💼 KvK & belasting** (vast kopje, Mark-wens 25 aug 2026): stand van de eenmanszaak-inschrijving/KvK-acties + relevante belastingregels en -deadlines (urencriterium/urenlogboek, startersaftrek-strategie, kosten-aftrek incl. AI-kosten, KOR, btw-id, aangifte-momenten) — bron memory `reference_eenmanszaak_belastingregels`; staande afspraak: aangifte sámen met Mark doornemen. **Plus sinds 25 aug (Mark-opdracht) twee levende administraties in dit kopje verwerken:** (a) **`docs/KOSTEN-2026.md`** — aanloopkosten-overzicht; bij élke mail-check nieuwe facturen/kostenposten erin zetten (let óók op doorgestuurde hotmail-facturen), en zolang er 📥-rijen openstaan in het dagrapport om de ontbrekende bedragen/facturen vragen (printer, enveloppen, Herma-stickers, TransIP, claude.ai-abonnementsfacturen); (b) **`docs/URENLOGBOEK-2026.md`** — urenlogboek voor het urencriterium: sessie-uren dagelijks bijwerken (voorzichtig schatten, nóóit opblazen — alleen échte uren, het logboek moet naast git/verzendlogs kunnen liggen) en in elk dagrapport Mark's offline uren van die week uitvragen (bellen/post/inkopen/gesprekken); meldt Mark tussendoor een klus in de chat ("2 uur flyers gepost") → direct loggen. Mark mikt op ~30 u/wk échte uren; eenmalige reconstructie mei-aug uit git/verzendlogs staat open (voorstel maken, Mark valideert); (7) 📋 Vervolgstappen (wie aan zet).
...
4. **📊 Cijfers-cockpit** — Noord-ster (via `events_mens`) als kopcijfer mét richting (↑/↓/=), daarna alléén de cijfers die bewegen of afwijken; alles wat 0/onveranderd is in één verzamelzin. Volledige trechter-detail alleen bij bijzonderheden.
...
LEFT JOIN (
  ...
  FROM events_mens WHERE name IN ('partner_bezoek','partner_actief') GROUP BY 1
) s ON s.code = pc.code
