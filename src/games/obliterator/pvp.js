...
export function subscribeMatch({ code, role, name, handlers = {} }) {
  // Mutabele handler-ref: PvPLobby start met onPresence/onStart, daarna geeft
  // ObliteratorGame z'n eigen onTick/onEnd door via setHandlers().
  ...
  channel.on("broadcast", { event: "tick" }, ({ payload }) => {
    activeHandlers.onTick?.(payload);
  });
  channel.on("broadcast", { event: "laser" }, ({ payload }) => {
    activeHandlers.onLaser?.(payload);
  });
  channel.on("broadcast", { event: "hit" }, ({ payload }) => {
    activeHandlers.onHit?.(payload);
  });
  channel.on("broadcast", { event: "start" }, ({ payload }) => {
    activeHandlers.onStart?.(payload);
  });
  channel.on("broadcast", { event: "end" }, ({ payload }) => {
    activeHandlers.onEnd?.(payload);
  });
  channel.on("broadcast", { event: "react" }, ({ payload }) => {
    activeHandlers.onReact?.(payload);
  });

  channel.on("presence", { event: "sync" }, () => {
    const state = channel.presenceState();
    activeHandlers.onPresence?.({
  ...
  return {
    channel,
    role,
    /** Vervang handlers (overgang lobby → game). */
    setHandlers(newHandlers) { activeHandlers = newHandlers || {}; },
