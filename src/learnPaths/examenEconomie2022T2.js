...
const steps = [
  ...
  },
  {
    ...
    checks: [{
      ...
      explanation: "**Incidentele uitgave** = eenmalige, niet-regelmatige uitgave. Aanschaf dier = doe je 1× (of zelden). Vaccinaties = jaarlijks (regelmatig). Voeding + zaagsel = wekelijks/maandelijks (regelmatig = vaste of variabele lasten).",
  ...
  },
  {
    ...
    checks: [{
      ...
      explanation: "Verzekering afsluiten heeft 2 effecten op huishoudboekje: **vaste lasten ↑** (elke maand premie betalen) + **incidentele uitgaven ↓** (onverwachte dierenarts-rekening wordt door verzekering betaald).",
  ...
  },
  {
    ...
    checks: [{
      ...
      explanation: "**Zelfvoorziening** = land/blok produceert zelf wat het nodig heeft (importeert minder). **Invoerheffing verhogen** = buitenlandse suiker duurder maken → consumenten kopen EU-suiker → EU-boeren leveren meer → zelfvoorziening stijgt. Quota verhogen = méér import (omgekeerd). Uitvoersubsidies = méér export (omgekeerd).",
  ...
  },
  {
    ...
    checks: [{
      ...
      explanation: "**CBS** = Centraal Bureau voor de Statistiek = meet wat werkelijk gebeurd is (achteraf). **CPB** = Centraal Planbureau = voorspelt (vooruit). Economische groei = groei van **nationaal inkomen** (= BBP) — niet besteedbaar inkomen (= wat huishouden overhoudt na belasting). Dus CBS + nationaal inkomen.",
