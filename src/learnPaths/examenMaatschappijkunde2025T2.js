...
const steps = [
  {
    ...
    checks: [{
      ...
      explanation: "**4 fasen politieke besluitvorming**: 1) AGENDA-vorming, 2) BELEIDSvoorbereiding (plan maken + inspraak), 3) BELEIDSbepaling (besluit nemen), 4) BELEIDSuitvoering (doen). Bestemmingsplan ter inzage = plan ligt klaar, burgers reageren = nog volop in voorbereiding.",
  ...
  },
  {
    ...
    checks: [{
      ...
      explanation: "**'Laatste woord'** = recht van de verdachte om aan het einde van de zitting (vóór vonnis) als laatste te spreken. Volgorde rechtszitting: tenlastelegging → verhoor → requisitoir (OvJ eist) → pleidooi (advocaat) → laatste woord (verdachte) → vonnis (rechter).",
  ...
  },
  {
    ...
    checks: [{
      ...
      explanation: "**Wetboek van Strafrecht** (Sr) = hoofdboek voor klassieke misdrijven: moord, diefstal, mishandeling, BEDREIGING, oplichting, vernieling. Specifieke wetten dekken aparte gebieden: Opiumwet (drugs), Wegenverkeerswet (verkeer), Wet wapens en munitie (wapens). Bedreiging = klassiek delict = Wetboek van Strafrecht.",
