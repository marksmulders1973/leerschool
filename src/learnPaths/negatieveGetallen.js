...
<text x="150" y="180" text-anchor="middle" fill="${COLORS.muted}" font-size="11" font-family="Arial" font-style="italic">negatieve getallen onder de knie 🏆</text>
    ...
    checks: [
      ...
      { q: "−8 + 3 = ?", options: ["−5","5","−11","11"], answer: 0, wrongHints: [null,"Klopt — 3 stappen rechts van −8.","Andersom — blijft onder nul.","Te ver naar links — heb je 3 erbij of eraf?","Wel onder nul."] },
      { q: "−5 − 7 = ?", options: ["−12","−2","2","12"], answer: 0, wrongHints: [null,"Klopt — verder naar links.","Niet — extra 7 erbij in min-richting.","Tegenovergesteld.","Te ver naar plus."] },
      { q: "12 − (−5) = ?", options: ["17","7","−17","−7"], answer: 0, wrongHints: [null,"Klopt — min van min = plus. 12+5=17.","Niet — twee minnen worden plus.","Niet — geen min.","Niet."] },
      { q: "(−4) × 5 = ?", options: ["−20","20","−9","9"], answer: 0, wrongHints: [null,"Klopt — min × plus = min. 4×5=20, met min.","Tegengesteld teken.","Niet vermenigvuldigen — optellen.","Idem."] },
      { q: "(−6) × (−3) = ?", options: ["18","−18","9","−9"], answer: 0, wrongHints: [null,"Klopt — min × min = plus. 6×3=18.","Tegengesteld — twee minnen = plus.","Niet — vermenigvuldigen, niet aftrekken.","Idem."] },
      { q: "24 ÷ (−4) = ?", options: ["−6","6","−96","96"], answer: 0, wrongHints: [null,"Klopt — plus ÷ min = min. 24÷4=6, met min.","Tegengesteld teken — 1 min in som = uitkomst min.","Niet — delen, niet vermenigvuldigen.","Idem."] },
      { q: "(−15) ÷ (−3) = ?", options: ["5","−5","45","−45"], answer: 0, wrongHints: [null,"Klopt — min ÷ min = plus. 15÷3=5.","Tegengesteld — twee minnen = plus.","Niet — delen, geen × .","Idem."] },
      { q: "Temperatuur −12 °C wordt 8 graden warmer. Nieuwe temperatuur?", options: ["−4 °C","+4 °C","−20 °C","20 °C"], answer: 0, wrongHints: [null,"Klopt — −12+8 = −4.","Te hoog — niet alle 12 graden weggewerkt.","Tegenovergesteld — warmer = optellen, niet aftrekken.","Veel te hoog."] },
      { q: "Een schuld van €25 + lening van €10 erbij. Totale schuld?", options: ["€35","€15","−€15","€25"], answer: 0, wrongHints: [null,"Klopt — schulden samen: 25+10=35.","Te weinig — extra geld geleend, schuld stijgt.","Met min — schuld is positief getal als 'schuld' genoemd; in saldo wel min.","Onveranderd — geleend erbij, dus hoger."] },
      { q: "Duiker op −15 m stijgt 9 m. Nieuwe diepte?", options: ["−6 m","−24 m","6 m","24 m"], answer: 0, wrongHints: [null,"Klopt — dichter bij zeespiegel (0).","Tegenovergesteld — stijgen = richting 0.","Niet — nog niet boven water.","Veel te hoog."] },
      { q: "Welk getal is HET GROOTST: −15, −3, 0, +2?", options: ["+2","−3","0","−15"], answer: 0, wrongHints: [null,"Klopt — meest naar rechts op getallenlijn.","Negatief, dus kleiner dan 0.","Wel groter dan negatieven, maar +2 is groter.","Het kleinst."] },
      { q: "**Verschil** tussen −7 °C en +5 °C?", options: ["12 °C","2 °C","−12 °C","−2 °C"], answer: 0, wrongHints: [null,"Klopt — afstand 0 → +5 = 5, plus 0 → −7 = 7. Samen 12.","Te weinig — vergeet niet de stappen onder nul.","Niet — verschil = afstand, altijd positief.","Idem."] },
      { q: "Reken: 10 − 4 × 3", options: ["−2","18","6","30"], answer: 0, wrongHints: [null,"Klopt — eerst ×: 4×3=12. Dan 10−12=−2.","Optellen ipv aftrekken.","Eerst −, dan × (verkeerde volgorde).","Geen haakjes — × eerst."] },
      { q: "Wat is **tegenovergestelde** van −9?", options: ["+9","−9","0","9 (al positief)"], answer: 0, wrongHints: [null,"Klopt — tegenovergestelde = teken omdraaien.","Zelfde getal.","0 is tegenovergestelde van 0.","Optie A is correct — '+9' notatie."] },
      { q: "Een lift gaat 3 verdiepingen omlaag vanaf etage 1. Welke etage?", options: ["−2","2","−1","4"], answer: 0, wrongHints: [null,"Klopt — 1−3 = −2 (parkeerkelder).","Tegenovergesteld — omlaag = min.","Slechts 2 omlaag.","Optellen ipv aftrekken."] },
    ],
  },
];
