...
export function sluitVergadering(st) {
  ...
  if (!top || gelijk) { uitkomst = { uitgestemd: null, tekst: "Geen meerderheid — niemand uitgestemd." }; }
    ...
    if (u.rol === "imposter") uitkomst = { uitgestemd: u, imposter: true, tekst: `${u.naam} was de nepper!` };
    else { st.fouteStemrondes += 1; uitkomst = { uitgestemd: u, imposter: false, tekst: `${u.naam} was géén nepper…` }; }
  }
...
export function checkEinde(st) {
  ...
  let u = null;
  if (impActief === 0) u = { gewonnen: "bouwers", reden: "De nepper is uitgestemd!" };
  ...
  else { const bouwersActief = actieveSpelers(st).filter((s) => s.rol === "bouwer").length; if (bouwersActief <= impActief) u = { gewonnen: "imposters", reden: "De neppers zijn in de meerderheid!" }; }
  if (u) {
