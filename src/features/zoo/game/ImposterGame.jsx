  ...
  useEffect(() => {
    ...
    const unsub = net.luister((van, d) => {
      ...
      if (host) {
        ...
        if (d.t === "rol" && d.voor === mijnId) { stRef.current = stRef.current || { spelerId: mijnId, spelers: [], stations: [], log: [], fase: "intro", takenKlaar: 0, takenTotaal: 0, spelTijd: 0 }; stRef.current.mijnRol = d.rol; // 10 sep: placeholder mét stations (rol kwam vóór de eerste snapshot → "stations is not iterable") const m = stRef.current.spelers?.find((x) => x.id === mijnId); if (m) m.rol = d.rol; return; }
