// 🎮 Wie doet alsof? — presentatie in de 3D-scène + HUD.
    ...
    return (
      <Html fullscreen zIndexRange={[12, 0]} style={{ pointerEvents: "none" }} calculatePosition={HUD_POS}>
        <div style={OVERLAY("rgba(10,20,40,.88)")}>
          <div style={KAART}>
            <div style={{ font: "900 22px system-ui", marginBottom: 6 }}>🎮 Wie doet alsof?</div>
            ...
            <div style={KAART}>
              ...
              {multi ? (
                ...
                <>
                  ...
                  <p style={{ margin: "0 0 8px", background: "#f4faf6", border: "1px solid #cde3d6", borderRadius: 10, padding: "8px 12px", fontSize: 13.5 }}>🤖 <b>Alleen spelen kan ook</b>, gewoon nu: vijf bots doen mee, en bij "Verras me" kan één van hen de nepper zijn. Je hebt niemand anders nodig.</p>
                  <p style={{ margin: "0 0 8px" }}>Zes spelers in het park: jij en vijf maatjes. Eén is de <b>nepper</b>. <b>Bouwers</b> doen taken bij de gele posten: drie vragen op jouw niveau. De nepper doet alsof en kan een bouwer <b>tikken</b> als niemand kijkt: die is af en kijkt mee vanuit de zeppelin. Wie uitgestemd wordt ook.</p>
                  <p style={{ margin: "0 0 8px" }}>Zie je iets verdachts? Druk op 🚨 en stem. Nepper uitgestemd of alle taken klaar = bouwers winnen. Tijd om of twee keer verkeerd gestemd = nepper wint. Elk goed antwoord = 10 punten, punten worden munten.</p>
                </>
              )}
              ...
              {(multi ? 1 + lobby.length + Math.max(1, 5 - lobby.length) : 6) >= 6 && (
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 8, alignItems: "center" }}>
                  <span style={{ font: "800 13px system-ui", color: "#556" }}>Neppers:</span>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
                {[["random", "🎲 Verras me"], ["bouwer", "🔧 Bouwer"], ["imposter", "🕵️ Nepper"]].map(([k, l]) => (
          ...
          <>
            {st.spelTijd < 4 && (
              <div style={{ position: "absolute", left: "50%", top: 70, transform: "translateX(-50%)", pointerEvents: "none", background: mij.rol === "imposter" ? "#b0332a" : "#1f7a3a", color: "#fff", borderRadius: 16, padding: "12px 18px", font: "900 20px system-ui", boxShadow: "0 8px 24px rgba(0,0,0,.4)", textAlign: "center" }}>
                {mij.rol === "imposter" ? "🕵️ Jij bent de NEPPER" : "🔧 Jij bent BOUWER"}<div style={{ font: "700 13px system-ui", opacity: .9 }}>{mij.rol === "imposter" ? "Doe alsof. Tik bouwers als niemand kijkt." : "Doe taken bij de gele posten. Let op wie er vreemd doet."}</div>
              </div>
            )}
        ...
        {st.fase === "vergadering" && st.vergadering && (
          <div style={OVERLAY("rgba(40,10,10,.6)")}>
            <div style={KAART}>
              ...
              <p style={{ margin: "4px 0 10px", color: "#556" }}>Wie doet alsof? Kies iemand, of onthoud je.</p>
              <div style={{ display: "grid", gap: 6 }}>
        ...
        {st.fase === "einde" && st.uitkomst && (() => { const sc = scoreVan({ ...st, spelerId: mijnId }); return (
          <div style={OVERLAY("rgba(10,20,40,.6)")}>
            <div style={{ ...KAART, textAlign: "center" }}>
              ...
              <div style={{ color: "#556", margin: "4px 0 10px" }}>{st.uitkomst.reden} De nepper was: <b>{st.spelers.filter((s) => s.rol === "imposter").map((s) => s.naam).join(" en ") || "…"}</b>.</div>
