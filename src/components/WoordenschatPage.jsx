...
const CATEGORIES = [
  { id: "synoniemen",  label: "Synoniemen",  icon: "🔁", color: "#42a5f5", desc: "Woorden met dezelfde betekenis" },
  { id: "antoniemen",  label: "Antoniemen",  icon: "↔️",  color: "#ef5350", desc: "Tegenovergestelde woorden" },
  { id: "betekenis",   label: "Woordbetekenis", icon: "🔍", color: "#ab47bc", desc: "Wat betekent een woord? + uitdrukkingen" },
];

export default function WoordenschatPage({ userName, studentProgress = [], onStart, onBack, onHome }) {
  const getStatus = (cat) => {
    const results = studentProgress.filter(
      (p) => p.player === userName && p.topic === `woordenschat ${cat}`
    );
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "0 16px 32px" }}>
          ...
          {CATEGORIES.map(cat => {
            ...
            return (
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontFamily: "'Fredoka', sans-serif", fontSize: 18, fontWeight: 700, color: "#fff" }}>
                    {cat.label}
                  </div>
                ...
                {score !== null && (
                  <span style={{ fontSize: 13, fontFamily: "'Nunito', sans-serif", fontWeight: 700, color: st === "green" ? "#69f0ae" : "#ffb74d", flexShrink: 0 }}>
                    {score}%
                  </span>
                )}
              </button>
        ...
        <button
          ...
          style={{
            ...
            border: "2px solid rgba(8,145,178,0.4)", color: "#22d3ee",
