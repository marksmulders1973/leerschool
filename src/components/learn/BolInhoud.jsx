...
export default function BolInhoud({ half = false }) {
  ...
  // patches (voetbal-look): posities relatief aan het middelpunt
  const patches = [[0, -0.05], [-0.45, -0.35], [0.45, -0.35], [-0.4, 0.35], [0.4, 0.35]];
    ...
    <div style={{ background: half ? "linear-gradient(160deg,#eef4ff,#dbe7fb)" : "linear-gradient(160deg,#f0f9f0,#dcefdc)", borderRadius: 16, padding: "14px 16px", maxWidth: 440, margin: "0 auto", color: "#243044", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        <svg viewBox="0 0 232 210" style={{ width: 206, height: 188, flexShrink: 0 }} role="img" aria-label={`${half ? "Halve bol" : "Bol"} met straal ${r} meter`}>
          ...
          {/* voetbal-patches */}
          {patches.map(([dx, dy], i) => {
            const py = cy + dy * R * (half ? 0.9 : 1);
            if (half && py > GROND_Y - 2) return null; // geen patch onder de rand
            return <circle key={i} cx={CX + dx * R} cy={py} r={R * 0.15} fill="#2b3550" opacity="0.82" />;
          })}
          {/* straal-maat */}
          <line x1={CX} y1={cy} x2={CX + R} y2={cy} stroke="#3a4a64" strokeWidth="1.4" strokeDasharray="3 2" />
          <text x={CX + R / 2} y={cy - 5} textAnchor="middle" fontSize="11" fontWeight="700" fill="#3a4a64">r {r} m</text>
        </svg>
