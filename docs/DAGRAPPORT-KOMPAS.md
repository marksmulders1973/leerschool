...
| 45 | **🎡 Interactief park — masterplan** (Mark 16 aug, TODO): uiteindelijk een interactief park voor álle leeftijden + álle leerpaden, in de stijl van de piramide (manipuleer + zwevende maten/formule + poort naar de les + aftelbaar). **Eerst een overall plan** zodat we het in één keer goed doen (generiek raamwerk, niet object-voor-object). Eerste plan: `docs/INTERACTIEF-PARK-MASTERPLAN.md`. Memory `idea_studiebol_interactief_park_masterplan` | 🟢 plan-v1 klaar, wacht op Mark's keuzes (fase 0) |
