...
**🔗 KOPPELING-IDENTITEIT (Mark-go 2 sep 2026 middag: "bouw het, in twee stappen") — ouder-kind = ruggengraat Familie**
- [x] ✓2/9 (v547) **Stap 1 — datamodel + schrijfpaden**: `link_id` op leaderboard/learn_progress/topic_mastery/ref_mastery + owner_key (link_id | uid:naam | naam:naam) met unieke sleutels; `src/shared/koppeling.js` bewaart link_id per kindnaam op het toestel na claim (KoppelcodeBanner + CodeBalk); leaderboardRepo/LearnPath/mastery schrijven link_id mee (metLinkId). Alles achterwaarts compatibel (nullable + fallback).
- [x] ✓2/9 (v548) **Stap 2 — leespaden + kind-banner** — DONE: claim_link_code gebruikt het ouder-label (Sophie/Sofie = één kind) en geeft child_name terug; klaargezet-RPC's + niveau-RPC + kandidaten-RPC op link_id (fallback naam+uid); nieuwe RPC koppel_mijn_data hangt eigen historie van het toestel direct aan de koppeling; kindData.js (OuderInzicht + KindOverzicht) en send-ouder-rapport lezen link_id + legacy; KoppelcodeBanner toont '✅ Gekoppeld met mama' + 'Andere code'. E2E via REST: code → claim (andere naam) → backfill → klaargezet → ouder leest 2 scores op link_id ✓. Nog niet: RLS-select voor ouder op progress/mastery (ouder leest via leaderboard+RPC's, dus niet nodig nu); kind kan koppeling nog niet intrekken. Was: OuderInzicht scores/cito, KindOverzicht, send-ouder-rapport, niveau-RPC (20260711) en klaargezet-RPC's lezen op link_id (fallback naam(+uid) voor rijen zonder link_id); RLS-select voor ouder op progress/mastery via link-eigendom; kind ziet "✓ gekoppeld met mama" + kan koppeling zien; naam-mismatch bij claim (ouder "Sophie", kind "Sofie") netjes afvangen (link_codes.child_name gebruiken); mastery `existing`-lookup op link_id.

**P1 — open**
- [x] ✓2/9 (v542) **F1 learn_progress unique op (player_name, pad, stap)** — DONE: generated column `owner_key` + unique (owner_key, pad, stap), oude constraint weg, 35 dubbele Speler/echte-naam-rijen samengevoegd, client onConflict aangepast; getest: 2e 'Testkind' met ander account slaat op, anon-upsert 201→200. Migratie `20260902_f1_learn_progress_owner_key.sql`. Was: → tweede kind met dezelfde voornaam kan nooit voortgang opslaan (upsert = UPDATE van andermans rij → RLS 403, live gezien bij "Testkind"). Plan: generated column `owner_key = coalesce(user_id::text, 'naam:'||lower(player_name))`, unique (owner_key, learn_path_id, step_idx), client `onConflict` daarop; reads in LearnPath/LearnPathsHub op user_id filteren als ingelogd.
- [x] ✓2/9 (v544) **F2 Partner-recht alleen in localStorage** — DONE: partner_claims.user_id + partner_codes.familie_tot (OOIEVAAR=null=blijvend); claim_partner_plek bindt aan auth.uid() (ook anonieme sessie; 'geen_sessie' zonder; 1 plek per account; legacy visitor-claims krijgen backfill); nieuwe RPC mijn_partner_recht(p_visitor) → {recht,code,org,blijvend,familie_tot}; useSubscription haalt het recht van de server (10-min-cache, 7-dagen offline-fallback), localStorage 'pro2027' is alleen nog UI-hint. ⚠️ PAYWALL_ACTIVE staat nog op false — bij aanzetten in jan één keer live testen met PRO2027. Was: (`useSubscription.partnerGrant`, `lk_partner_status`) → vervalsbaar zodra paywall aan; `claim_partner_plek(p_visitor)` client-gekozen → plekken leeg te trekken. Vóór paywall: entitlement server-side (partner_claims aan auth.uid()).
- [x] ✓2/9 (v542) **F3 Onbekende partnercode via QR-URL wordt geaccepteerd** — DONE: vangPartnerCode checkt achteraf via partner_code_bestaat en wist een onbekende code (mits geen plek geclaimd); PartnerWelkom toont 'Deze code kennen we niet'-kaart zonder gratis-belofte. Was: (`vangPartnerCode` valideert niet; `?partner=TEST123` toont volledig welkomstscherm + belofte). Fix: `partner_code_bestaat`-RPC vóór PartnerWelkom, anders "code onbekend".
- [x] ✓2/9 **F4 Klaargezet-RPC's matchen op voornaam voor anon** — DONE (live + migratie 20260902_f4_f5): 4 RPC's met `(child_user_id is null or = auth.uid())`. Was: (`voor_jou_klaargezet`, `kind_klaargezet_gedaan`, `leerling_klaargezet_gedaan`) → elke "Sophie" ziet/vinkt elkaars lessen. Fix: `and (child_user_id is null or child_user_id = auth.uid())`.
- [x] ✓2/9 **F5 Koppeling hangt aan vluchtige anon-uid** — DONE: claim_link_code slaat alleen niet-anonieme uid op; 3 test-koppelingen aan anon-uid → null (naam-match). Ouder-kant (OuderInzicht/KindOverzicht/ouder-rapport) had al naam-fallback bij null. Was: (`claim_link_code` zet child_user_id = anon-id; 1 van 4 koppelingen wijst naar anon) → ouder ziet 0 na herinstallatie. Fix: child_user_id alleen bij niet-anonieme sessie, anders naam-match.
- [x] ✓2/9 (v545) **F6 "Verwijder al mijn data" wist niet alles** — DONE: RPC wist nu dynamisch álle user_id-tabellen + parent/teacher/maker-rijen + pvp + e-mailgebonden rijen, maakt child/student_user_id in andermans koppelingen los, en verwijdert profiel + auth-account; client logt na 2,5 s uit en wist lk_/ls_-sleutels; copy aangepast. E2E getest met wegwerp-anon-account (3 rijen + account weg). Migratie-notitie 20260902_f6. Was: (learning_goals, diagnostic_assessments, daily_plans, leraar_leerling_links, referral_codes, auth.users blijven) — RPC uitbreiden of copy versmallen (AVG/DPIA!).
- [x] ✓2/9 (v550) **F7 Resend-daglimiet (100) op maandag in toetsseizoen** — DONE: gedeeld DAGBUDGET=95 in send-weekly-lesmateriaal (ouder-rapport eerst/ongelimiteerd, daarna kwartiercheck → aftelreeks → weekmail met maxMails = rest); wie niet past blijft 'due' en gaat de volgende run mee; 'dagbudget-op' in fouten-lijst van het dagrapport. Was:: ouder-rapport + kwartiercheck + aftelreeks + weekmail in één run; nu 24 op lijst dus oké, maar bij groei structureel. Gedeeld dagbudget of aftelreeks op andere dag.
- [x] ✓2/9 (v549) **F8 AI-endpoints: inputlengte niet afgekapt** — DONE: generate-questions klemt subject/level/topic/textbook-velden (60/40/200/160); preview-topic String-cast + 120; tutor-chat klemt pathTitle/stepTitle/checkQuestion/opties(max 6)/lastWrongAnswer en filtert null-berichten; dailyQuotaCheck in alle 6 endpoints pas ná body-validatie (lege POSTs met gespoofde Origin vraten anders het quotum). Was: (generate-questions `topic`/`level`, tutor-chat context-velden, preview-topic) + quota telt vóór body-validatie (`_guard.js`). Één sweep: `slice(0,200)` per veld, `level` alleen uit map, quota pas vóór Anthropic-call.
- [x] ✓2/9 **F9 Repo-migraties lopen achter op live DB** — DONE: `supabase/migrations/20260902_snapshot_live_schema.sql` = volledige live-snapshot (tabellen/views/functies/triggers/policies/grants, ~100 KB) via nieuwe DB-functie `admin_schema_snapshot()` (alleen SQL-editor/MCP). Vernieuwen = één query + bestand vervangen. Was: (klaargezet-RPC's, partner_codes/claims, referral_codes, koppelcode-policies niet in `supabase/migrations/`). Export-migratie maken uit pg_policies + pg_get_functiondef.
...
- [x] ✓2/9 (v559) **F15 double opt-in** — DONE: `upgrade_waitlist.confirmed_at` (eerste mail = het gevraagde ding + bevestig-knop, reeks/aftelreeks/leesladder pas na tik op `/api/bevestig?token=`; 26 bestaande leads = bevestigd); partner-meelezen via `/api/partner-uitnodiging` (JWT) → uitnodigingsmail → `/api/bevestig?partner=` → RPC `ouder_weekrapport_kandidaten` geeft het adres pas ná bevestiging. ⚠️ Hosting-les: Hobby = max 12 gebundelde functies — 23 losse api-bestanden faalden, 21 werkt; nieuwe endpoints dus als `api/_lib/*.js` + rewrite via een bestaande functie (hier `unsubscribe.js?actie=`). Live E2E groen (uitnodiging → token → bevestig → RPC). Was: send-oefenblad/weekpakket-code: derden inschrijven zonder bevestiging (single opt-in); partner-mailadres ouder idem.
...
- [x] ✓2/9 (v554) F17 — DONE (minimale, veilige variant): isLaunchPromoActive() blijft true zolang PAYWALL_ACTIVE=false, dus geen dichtklappende Familie-poort op 1-1-2027 zonder betaalroute; bij paywall-aanzetten in jan één bron (useSubscription) doortrekken. Was: Drie entitlement-bronnen (isLaunchPromoActive / PAYWALL_ACTIVE / subscriptions) → tijdbom 1-1-2027; één bron.
- [x] ✓2/9 (v554) F18 — DONE: chapter D-bereik verruimd in argumentatieleer (→14), doorstroomtoets-rekenen-g8 (→5), doorstroomtoets-taal-g8 (→4); 100% is weer haalbaar. (21 manifest-paden niet in ALL_LEARN_PATHS = blijft open, laag risico). Was: Stappen buiten hoofdstuk-bereik: argumentatieleer s15, doorstroomtoets-rekenen-g8 s5-6, doorstroomtoets-taal-g8 s5 (100% onhaalbaar); + 21 manifest-paden niet in ALL_LEARN_PATHS.
- [x] ✓2/9 (v554) F19 — DONE: voortgang alleen over getoonde paden, per pad afgekapt op stepCount; 'Doorgaan' gebruikt eerst loadResume (laatst bezochte plek). Was: LearnPathsHub voortgang% kan >100% (telt alle rijen, deelt door gefilterde paden); "Doorgaan" = manifest-volgorde i.p.v. laatst bezocht.
- [x] ✓2/9 (v560) **F20 Vragen-kwaliteit** — DONE: 3745 → "het ijs ___ gesmolten" (is); 710 → "Lisa … haar moeder"; textbook 318 → 2499 i.p.v. 3499; breukenPo 83 → 2/6 i.p.v. 2/3 (+ "Cito"→"de toets"); G8 2260 Tom-som → "drie vrienden delen €18"; "zowel b als c" (1902 Duits → "Wie heißt du?", 2860 g → 9,8 m/s², 2905 vermogen → P = U × I); 625 -heid, 630 rijdt, 632 hond-d, 636 bijwoord-in-zin, 640 dubbele ontkenning herschreven; 1211 → langstzittende premier (Rutte). Niet gevonden: socialVragen "5 opties, 2 goed" — bestand heeft overal 4 opties (scan), mogelijk verkeerd bestand in de review. Was: ambigu — sample 3745 (heeft/is/had), 710 (haar/zijn/hun), textbook 318 (2987/3499), socialVragen 343 (5 opties, 2 goed), breukenPo 83 (4/6=2/3), doorstroomtoetsRekenenG8 2260 (Tom-som); sample 1902/2860/2798 "zowel b als c" na optie-shuffle kapot; 625/630/632/636 kapotte vraagstelling; 1211 datum-gevoelig.
...
- [x] ✓2/9 (v561) **F23 Cito → Doorstroomtoets** — DONE: sweep over álle src-teksten (4 rondes, ~2.230 vervangingen in 200+ bestanden): Cito-truc/-tip/-feitje → Toets-…, Cito-stijl/-mix/"(Cito groep 7-8)" → Doorstroomtoets-…, "Cito vraagt/test/heeft" → "de toets …", kale "Cito" in zinnen → "de toets"/"de Doorstroomtoets"; manifest + landingspagina's hergenereerd (npm run build). Bewust gelaten: aanbieders-uitleg (**Cito** (meest gebruikt), IEP…), examen-bronnen ("Stichting Cito"), "Cito Leerling in Beeld"-zoektip, eigennaam-voorbeeld, code-identifiers/comments, CSE-paden "Cito-/HAVO-…". Rondleiding: "eind januari, begin februari (2027 vanaf 25 jan)" = MijnPagina. 15-min-gate: interpunctie-po 20→25m door langere woorden → baseline herijkt. Was: "Cito" nog 183× user-facing in doorstroomtoets-G8-paden + ExamensPage/StudentHome "Cito-toets"; RondleidingPage "derde week januari" vs elders "begin februari".
...
- [x] ✓2/9 (v546) **F28 (P1) "Leg het uit" zonder pad-match landt nog steeds op /komt-eraan** — DONE: (a) PlayQuiz: geen pad-match + vraag heeft uitleg → handleAnswer(-1,{gaveUp}) = uitleg ín de quiz (gele "Geen probleem"-balk, telt als niet-geweten), alleen zonder uitleg nog naar "komt eraan"; (b) map-generator gerepareerd: esbuild-bundel van learnPaths/index.js (jsx + import.meta.env), vragen uit src/data/, niveau-filter (groep→PO, klas→VO) + woordgrens voor korte keywords + topic-naam-niveau; map herbouwd: 5.145 entries, PO-vragen nu naar -po-paden (breuken→breuken-po, "0,8×0,5"→kommagetallen-po); 589 oude ruis-koppelingen weg (√81 groep 7→VO-pad etc. = F21-content). Was: — mapping-fix v541 geeft nu wél "37 leerpaden klaar"-knop, maar bv. "0,8 × 0,5" heeft geen keyword-match. Beter: bij noMatch de uitleg van de vraag zelf in de quiz tonen (zelfde overlay als bij fout antwoord) i.p.v. wegnavigeren. Plus: `scripts/match-questions-to-paths.mjs` is kapot (ERR_UNKNOWN_FILE_EXTENSION, importeert constants.js dat niet meer bestaat) → map van 5.115 entries is verouderd; generator repareren + herdraaien nu er 37 rekenpaden zijn.
...
- [~] **E1** biologie-2023-T1: ✓10/8 (v238) **6→9 speelbaar**: ① ethogram-tabel gereconstrueerd (tabel-route) + vraagtekst opgeschoond · ② preening-vraag: dode beeld-referentie zat alleen in plak-context → context ingekort, nu speelbaar; explanation "antwoord is C" vervangen door echte uitleg · ③ oog-doorsnede: „Human Eye Transverse Cut (unlabeled)", Andrewmeyerson, CC BY-SA 3.0 + P-T-letters (T=hoornvlies ✓ officieel antwoord) · ④ tand-doorsnede: „Tooth Section numbered", Jak, publiek domein — cijfers→letters in de svg zelf, wees-lijnen/accolades verwijderd (T=tandbeen ✓). ⑤ NIEUW vangnet: vlag `beeldOntbreekt` + filter in index.js voor beeld-vragen die het woordfilter missen (bloedvat-letter-vraag i5 zo eerlijk uitgefilterd — die glipte er vóór deze sprint al doorheen). REST naar 13 (volgende sessie): i1 voortplantingsstelsel (anatomie-plaat Wikimedia zoeken, P=afsluiting bij blaas), i2 voedselweb + i4 koningsvis-grafiek + i5 bloedvat (échte examen-PDF als bron nodig voor getrouwe reconstructie — examenblad.nl bio GLTL 2023-T1 downloaden).
- [ ] **E2** economie-examens (⚠️ analyse 10 aug: EERST bron-PDF's van examenblad.nl downloaden — eco-2023-T1 i5 = marketingmix-advertentie (niet namaakbaar, bron nodig of weglaten) en i8 hangt op "informatiebron 12" (zonder die bron is de winstmarge-redenering gokwerk; NIET reconstrueren op aannames). Zelfde geldt vermoedelijk voor de andere eco-grafiek-vragen. URL-patroon: memory reference_examenblad_urls). economie-2025-T2: 7→? · economie-2023-T1: 7→9 · economie-2025-T1: 9→11
...
- [x] ✓30/7 **Q1 (P1, 1 min)** index.html:432 — FAQ-kop mist "Wat": `Is het verschil tussen Cito-eindtoets en Doorstroomtoets?` → "Wat is het verschil tussen de Cito-eindtoets en de Doorstroomtoets?" (prio-#1-zoekcluster!)
- [x] ✓30/7 **Q2 (P1, 5 min)** public/abonnement.html — r216 "echte begrip"→"écht begrip"; r83 "engels"→"Engels"; r203/219 "Auto-renewal"/"paraphrase"→"automatische verlenging"/"parafrase".
- [x] ✓30/7 **Q3 (P1, 15 min)** src/learnPaths/begrijpendLezenStrategie.js — 6 kind-zichtbare taalfouten: r127 "accept"→"accepteer", r271 "De eindslot", r287 "middenste"→"middelste", r358 "de hoofdpunt"→"het hoofdpunt", r367 "hele"→"helemaal", r666 kapotte vraagzin herformuleren. Nota bene hét begrijpend-lezen-pad.
- [x] ✓30/7 (bestanden; drukorder liep al) **Q4 (P1)** Gratis-claim flyers: `_template-flyer-b1.html:94` + ALKMAAR2027 + HAARLEMMERMEER2027(-DRUK) zeggen "Helemaal gratis. Geen kosten achteraf." (kaal, tegen eigen regel). Sjabloon + bestanden herformuleren ("gratis voor uw gezin t/m de Doorstroomtoets 2027" o.i.d.). ⚠️ HAARLEMMERMEER-DRUK is 29 jul al bij de drukker besteld — checken of nog aanpasbaar; zo niet: geen ramp (r78 kwalificeert wél), maar sjabloon fixen vóór volgende order. Ook "goodybag"→"goodiebag" in dezelfde flyers + bedankt.html + voor-organisaties.html.
- [x] ✓30/7 (40 indexes + sitemap 357) **Q5 (P1)** 238 examen-vraagpagina's hebben kruimelpad-links naar 40 niet-bestaande set-overzichten (/examen/<vak>/<set>/ → SPA-home). Fix: per set een index.html genereren (uitbreiding buildPadLandingsPaginas.mjs-patroon) óf kruimel tijdelijk naar /vmbo-examens-oefenen.html.
- [x] ✓30/7 (CSP verruimd) **Q6 (P1)** Translate-balk kapot: eigen CSP (vercel.json:34) blokkeert translate.google.com voor niet-NL-bezoekers → dropdown laadt nooit. Domeinen aan CSP toevoegen óf widget verwijderen (index.html:528-580).
- [x] ✓1/8 **Q7 (P2, groot)** 773 antwoord-verklappende wrongHints in 97 files herschreven naar denkprikkels → `lint-wronghints` exit 0 (0 files, 0 hints), build groen. Aanpak: ICP-kern `doorstroomtoetsRekenenG8.js` (52) zelf gedaan als referentie (ving 3 échte antwoord-verklappers: r.889 `1÷4=25% en 3÷4=75%`, r.992 `250÷10=25`, r.1136 `(10/40)×100=25`); resterende 96 files via 9 parallelle sonnet-agents met disjuncte sets + zelf-verificatie (`_wh-detail.mjs` tot Totaal:0 + `node --check`). Tientallen hints bleken het JÚISTE antwoord te verklappen (bv. `balansBeco (20.000−0)/4=5.000`, `continentenWereldPo "Cito-standaard = 7"`, `pythagoras √289=17`) — nu allemaal denkprikkels. Regel: alleen strings binnen `wrongHints:` gewijzigd; `explanation`/`uitlegPad`/`niveaus` ongemoeid (mogen antwoord bevatten).
- [x] ✓1/8 (vak-label-mapping /leren afgerond: `CITO_PIJLERS` in LearnPathsHub.jsx miste de losse subjects → **wereldorientatie + verkeer** onder Wereldoriëntatie en **spelling** onder Taal gezet, 17 eerder-onvindbare paden nu in de pijler-filter; label 'Wereldoriëntatie' stond al goed via subjects.js. **Follow-up voor Mark:** `studievaardigheden` (7 paden) valt óók buiten élke pijler — het is een eigen Doorstroomtoets-onderdeel; overweeg een 5e pijler-knop. Rest ✓30/7) **Q8 (P2, klein-batch copy)** "Altijd gratis" in ProPage.jsx:178 + proPlan.js:68 (tegen eigen regel in zelfde bestand!); dev-jargon "uitlegPad" in index.html:439 + doorstroomtoets-oefenen.html; "database" in PlayQuiz.jsx:888; "Cito-toets" als toetsnaam in 3 leerpaden; "teveel"→"te veel" projectbreed; "Quizzes"→"Quizzen" (index.html:403); afgebroken zinnen + "AT" in begrijpendLezenTekstenPo.js (r243-367); vak-labels /leren ("wereldorientatie"→"Wereldoriëntatie", losse categorie "wereld" samenvoegen); kwartiercheck.html "Volledig gratis" kwalificeren.
- [~] deels ✓30/7 (c origin-guard, e JSON-LD samengevoegd, f helper→_lib; open: a layout-shift, b dubbele calls, d soft-404) **Q9 (P2 tech)** (a) /leren continue layout-shift (Playwright-clicks >5s instabiel — CLS-risico); (b) dubbele Supabase-calls per pageload (subscriptions/profiles 2×GET + 2×PATCH); (c) origin-guard: ontbrekende Origin valt terug op vervalsbare Referer (api/_guard.js:37); (d) soft-404 (routes.js:134 fallback home → nette 404-pagina + noindex); (e) 2 tegenstrijdige WebApplication-JSON-LD-blokken op home (index.html:84-133, featureList bevat ook niet-live claims — eerlijke-getallen-regel); (f) api/send-kwartiercheck-week.js (helper) wordt als publieke 500-endpoint gedeployed → naar api/_lib/.
- [~] deels ✓30/7 (manifest + og:url; open: titles/descriptions/twitter-cards/audit-scripts) **Q10 (P3 klein)** leesladder-title 92 tekens inkorten; 6 lange meta-descriptions; twitter:card+og:site_name op leesladder/oefenpakket/begrijpend-lezen-oefenen; og:url-slash home; manifest screenshot-maat + maskable-iconen; audit-scripts: jaartal-false-positives audit-units.mjs + 4 helpers in SKIP_FILES auditKennisgraaf.mjs.
...
- [x] ✓31/7 **P1 — E-mail-tagline** (v141): gedeelde helper `api/_lib/mail-tagline.js` → *"De basis van Leerkwartier — oefenen en uitleg — is en blijft gratis."* + funnel-link, onder alle 7 templates (welkom/week, ouder-rapport, doorstroom-countdown, leesladder, oefenblad, kwartiercheck + vervolgweek). Verving de nu misleidende "In 2026 is alles gratis"-regels. Eén bron van waarheid = de helper.
- [x] ✓31/7 **P2 — "Jouw pakket"-badge óók bij de naam** (v142): klikbare pill direct onder "Hoi <naam>" in StudentHome → toont huidig pakket ("Jouw pakket: Gratis") + opent de uitleg-modal. Gekoppeld aan `useSubscription` + TIERS: paywall-uit = "Gratis" voor iedereen; mapt later naar "Familie"/"Pro" (groen/goud). "Beiden" nog toe te voegen als iemand Familie én Pro heeft.
- [x] ✓31/7 **P2b — Prijs-ingang op de publieke home** (v143, Mark-vraag "staat de prijs ergens duidelijk op de home?"): de zichtbare marketing-home (HomePage.jsx) toonde géén prijs/link (de FAQ met "gratis" zit in het verborgen SEO-blok). Nu: discrete "💶 Wat kost het?"-knop in de footer-rij (naast Voor ouders/Voor leerkrachten) → opent ProPage (Gratis-basis + Familie/Pro/Kwartier-tegoed, eerlijk "vanaf 2027"). Ook crawlbare `/abonnement.html`-link toegevoegd aan het SEO-blok. Live-geverifieerd: knop aanwezig + klik → /pro.
...
- [x] ✓31/7 **V1** Her-verificatie ronde-1-fixes — visueel bevestigd (gerenderd + gescreenshot, niet aangenomen): voetregel-doorstreep ✓, hormonen-labels + ♀/♂-split ✓, planeet-labels alternerend + Pluto ✓, kaarten (Den Briel west, 17 Provinciën) ✓, WO2-Europakaart ✓, oorzaken-schema's ✓, onder-leden→ontleden ✓, AT→BIJ ✓, klijn herkaderd als niet-woord ✓. Resterend probleem gevonden + meteen gefixt (v135): tachtigjarige-oorlog- én WO2-tijdlijn hadden nog label-overlap in dichte jaar-clusters → nu gelijk-verdeelde events (agent). Tool: scripts/verify-visuals-v1.mjs (rendert paden groot voor visuele check).
- [x] ✓31/7 **Beauty-ronde 1 (Mark: "agents op de visuals zetten en verbeteren, zoals NL-kaart/fotosynthese")** — fotosynthese herbouwd tot heldere in/uit-cyclus (plant-centrum, getekende zon, invoer/uitvoer gescheiden, geen clipping na mijn viewBox-fix); 2 geschiedenis-tijdlijnen strak (zie V1). Verificatie = ik render+screenshot elk resultaat zelf (agents zien geen vormen).
- [x] ✓1/8 **V2** Eurozone 20→21 (Bulgarije per 1-1-2026): 8 vindplaatsen in 6 paden bijgewerkt — europeseUniePo (kop-theorie + quiz-check: antwoord + opties naar "21 landen" zodat het klopt + euro-landenlijst incl. Bulgarije), financieleVormingPo (quiz-check + geld-uitleg + Cito-vraag), nederlandseStaatMaatschappijleer (landenlijst +BG), fransCseHavoVwo (2×), bbpConjunctuurEconomie, wereldorientatieMixPo (2×). Bulgarije overal uit de "eigen munt"-lijst (7→6). **Bewust ongemoeid:** examenEconomie2024T1 (expliciet 2024-gestempeld — 20 wás toen correct; authentiek-examen-context).
- [~] **V3 — BEAUTY-QUEUE (3 scouts 31 jul, gerangschikt; werkwijze: improvement-agent + Claude als visuele keurmeester render+screenshot elk resultaat).** Batches, hoogste waarde eerst:
  - **B1 — Concrete Cito-kern-contexten (rekenen/natuur groep 6-8, iconische quick-wins) ✅ AF (v136-138):** thermometer negatieve getallen · ✅ deeltjesmodel vast/vloeibaar/gas (3 panelen rooster→los→verspreid + ijs/water/stoom) · ✅ tafels als stippen-arrays (3×4 = 3 groepjes van 4) · ✅ spelling lettergrepen in gekleurde blokjes (ta/fel + man/nen, kort/lang-klinker).
  - **B2 — Biologie cel & moleculen (scheikunde; coherente set, grootste herkenbaarheids-sprong):** chemische reactie als bol-atomen ipv 2 rechthoekjes (chemischeReactiesScheikunde) · echte plantcel-vorm + vacuole + chloroplasten (celBiologie) · organellen als vormen (mitochondrion-boon, ribosomen-clusters) · plantcel vs dierlijke cel naast elkaar.
  - **B3 — Biologie voortplanting/hormonen ✅ AF (v145):** menstruatiecyclus herbouwd als ronde 28-dagen-klok (4 gekleurde fase-bogen op de ring + ovulatie-punt dag 14 + dag-markers + legenda "↻ na dag 28 begint dag 1 opnieuw"; vervangt de platte tijdlijn) · nieuwe `hormoonRouteSvg` klier→bloedbaan→doelorgaan (moleculen door de bloedbaan + receptor/sleutel-slot) op stap A1 · organen (vrouw/man) beoordeeld = al herkenbaar, schoon gerenderd, ongemoeid gelaten. Live-geverifieerd via preview-screenshot.
  - **B4 — Aardrijkskunde/maatschappij ✅ AF (v146):** Tweede Kamer als halve-cirkel-zaal (`tweedeKamerSvg`: 150 zetels in een boog, linkerwig van 76 gekleurd = meerderheid; vervangt het blokjes-raster) · Trias Politica met iconen (`triasPoliticaSvg`: boek=wetgevend, vuist=uitvoerend, weegschaal=rechtsprekend) · platentektoniek plaatgrenzen herbouwd tot echte dwarsdoorsneden (`plaatgrenzenSvg`: oranje mantel-band + dikke platen-slabs + bewegingspijlen + landschap per type: rug/spleet-magma, subductie+berg, breuklijn). Live-geverifieerd via preview-screenshot.
  - **B5 — VO-niche (laag):** economie vraag/aanbod-grafieken kleur + evenwichtsster · argumentatieleer boom-structuur + iconen · logaritmen mini-grafiek.
  Volledige cel-verwijzingen in de 3 scout-outputs (task-ids aa30db9b/af20cf1b/a2f2031d, 31 jul).
...
- [x] **A8.2 Begrijpend lezen 3 → 15 paden — ✅ COMPLEET 15/7.** Batch 1 ✓8/7 (verwijswoorden / woordbetekenis-context / conclusies-trekken / tekstdoel-schrijversdoel). Batch 2 ✓15/7 (feiten-details 17 / oorzaak-gevolg 16 / alinea-tussenkopjes 17 / nieuwsbericht-5W+H 17, commits 4fdb67e..d3c32ce). Batch 3 ✓15/7 (brief-email 17 `aa6ab3c` / schema-tekst-combi 17 `1f86b6e` / lange-toets-teksten-g8 18 `e9919e0` / verhaal-diepte 17 `b46774a`). Totaal 15 paden, ~250 checks, alle linters schoon, gate OK, 219 tests + build groen, manifest 334 paden (`7fe483c`). Zelf-test op productie gedaan (nieuwsbericht-pad, fout-flow ✓). NB batch-3-agents raakten Max-sessielimiet ná het schrijven — bestanden bleken compleet en gevalideerd.
- [x] ✓15/7 **Trio-uitbreiding gered uit working tree** — +1623 regels vragen voor doorstroomtoets taal (werkwoordspelling-stap) / studievaardigheden (kaart & legenda-stap) / rekenen stonden ongecommit op schijf (eerdere sessie, nooit gecommit). Gevalideerd (219 tests + build + linters) en gecommit (4767a30); duur-baseline herijkt (trio-pools bewust groot).
- [x] ✓15/7 **A8.3 Streak + dagdoel naar Supabase** (commit 0779238) — 5 kolommen op profiles (kwartier_streak/_best/_last_day + goal_day/goal_seconds, migratie `kwartier_streak_dagdoel_op_profiel`), pure merge `mergeDailyGoalStates()` (+6 tests), sync-init in useAuth (geen extra roundtrip), push direct-bij-voltooid anders 1×/min. Anoniem = ongewijzigd lokaal. NB: dubbel-tel-risico afgedekt door bestaande completed/lastCompletedDate-guards.
- [x] ✓23/7 **A8.4 A12 web push LIVE** — samen met Mark: VAPID-keys, Vercel env (in bundle v94 geverifieerd), 3 Supabase-secrets, migration, edge function ACTIVE (mét verify_jwt; cron gebruikt anon-Bearer), pg_cron 15:00 UTC, privacy-tekst. Test-invoke ok. Open: Mark abonneert zichzelf via 🔔; abonnee-teller in dagrapport. Bijvangst gefixt: lege SUPABASE_SERVICE_ROLE_KEY in Vercel gevuld (kosten-cap weer actief, v95).

## 📈 SPRINT — Conversie & retentie (Mark "ja" 23 jul, uitvoering later)

> Uit score-gesprek 23 jul (product 8 / tractie 3): alles wat tot feb gebouwd wordt
> moet conversie of retentie raken. Twee taken door Mark goedgekeurd, nog te bouwen:

- [x] ✓28/7 **C1 E-mailcapture op /cito + /leren/pad** — oefenpakket-prompt op de 2 drukste pagina's (conversielek: <1% van bezoekers laat e-mail achter). Verkenning 23 jul: hergebruik `upgrade_waitlist`-insertpatroon uit `OefenpakketPage.jsx`/`GratisLesmateriaal.jsx`; doelcomponenten `CitoPage.jsx` + leerpad-speler (route /leren/pad in App.jsx ~r239). Compact bouwen: 1 herbruikbaar `<EmailCapture bron="..."/>`-blok, niet opdringerig (onder de content), event loggen, lijst-groei verschijnt al in dagrapport.
- [x] ✓28/7 **C2 Noord-ster-metric in dagrapport** — "gezinnen/apparaten die afgelopen 7 dagen op ≥2 dagen oefenden" als vast kopcijfer. LET OP verkenning 23 jul: `events` heeft géén user-id (session = per-tab) → eerst mini-fix: anoniem willekeurig **apparaat-kenmerk** meesturen in track() (géén naam/e-mail; privacy.html-alinea aanpassen: "willekeurig apparaat-kenmerk" i.p.v. alleen sessie-kenmerk) → daarna SQL in dagrapport-ritueel + CLAUDE.md.
- [x] ✓28/7 **C3 Gratis Kwartiercheck — diagnostische lead-magnet** *(Mark 28 jul: "echt diagnostisch", prio na C1)*
...
- [ ] **✨ Trots-momenten-preview: voorbeeldnaam "Sophie" → echte kindnaam indien bekend (Mark 28 aug avond, "sla op voor morgen").** Op de kennismakingspagina `src/features/familie/TrotsMomentPagina.jsx` staat het eerste voorbeeld-kaartje hardcoded op "Trots-moment voor **Sophie**!" (`VOORBEELDEN`, regel 12). De échte in-flow `<TrotsMoment>` gebruikt al wél de echte naam (`voor ${wie}`). Wens: als we een kindnaam kennen, die in het voorbeeld tonen i.p.v. Sophie. **Aanpak:** pak een kindnaam van dit apparaat — eerste niet-ouder/niet-leerkracht-profiel uit `lk_namen` (rol uit `lk_profiel:<naam>`), anders een verzorgde fallback ("Sophie" of neutraler "je kind"). Alleen het eerste kaartje personaliseren; streak/onderwerp-kaartjes generiek laten. Klein & veilig (geen datamodel), 1 bestand. Let op: op de Familie-hub kijkt vaak de ouder → `player` is dan de oudernaam, dus NIET `player` gebruiken maar de kind-profielnaam. Eén regel: pas het `titel`-veld van `VOORBEELDEN[0]` dynamisch aan in de component.

- [x] **🔴 BUG (Mark 28 aug, zelf gezien): ouder-klaarzet-modus opent de les zelf i.p.v. klaarzetten.** ✅ **GEFIXT v500-501 (zelfde avond, live getest met test-ouder-login):** 3 oorzaken gevonden: (1) tegel-onClick riep altijd `onPickPath` → les startte, alleen het kleine hartje zette klaar → nu: in klaarzet-modus toggelt de héle tegel (klaarSet + toggle op hub-niveau, KlaarzetHart controlled); (2) "▶ Doorgaan waar je was"-knop + leerpad-bot openden ook direct een les → verborgen in klaarzet-modus; (3) de klaarzet-balk ontbrak op het vák-keuze-scherm (eerste scherm!) → ouder zag niet eens dat de modus aanstond → balk nu gedeeld op alle hub-schermen. Bonus: rol-menu-knop "Zet oefenwerk klaar" op /mijn was een stille no-op (onKlaarzetten zonder linkId) → gaat nu naar de ouder-weergave. Live geverifieerd op productie: tegel-tik = 💛 aan + rij in `ouder_klaargezet` + zichtbaar via `voor_jou_klaargezet` RPC; 2e tik = weg; hartje direct = werkt; ✓ Klaar → terug naar /ouder. Test-account `test-klaarzet-ouder@leerkwartier.app` + kind "Testkind Klaarzet" aangemaakt (in household-filter) — herbruiken voor de test-sprint hieronder. Repro volgens Mark: als ouder in het thuis-overzicht "💛 zet lessen klaar" gebruikt → hij kwam in de vrágen terecht (les startte) in plaats van de klaarzet-modus in de hub (sticky banner + 💛-hartjes per tegel). Verwacht gedrag: `startKlaarzetten` → LearnPathsHub met `klaarzetVoor` + `KlaarzetHart`; tegel-tik zet klaar, start géén les. Onderzoeksrichting: (a) wordt `klaarzetVoor` wel gezet/doorgegeven vóór de navigatie (App.jsx → LearnPathsHub props)?; (b) valt de tegel-klik door naar de normale onOpenLes-handler i.p.v. het hart?; (c) mogelijk regressie sinds v314 (leerkracht-generalisatie `klaarzetVoor.bron`). Zie memory `project_studiebol_ouder_klaargezet`. **Eerst 1-2 repro-details bij Mark checken (welke route: /mijn of /ouder, welke knop), dan pas fixen** — genoteerd 28 aug omdat Fable-verbruik op 98% zat; oppakken in de volgende sessie mét kliktocht-agent op de klaarzet-route.

- [ ] **🔴 SPRINT — Klaarzet/huiswerk-keten VOLLEDIG doortesten, álle rolcombinaties (Mark 28 aug: "dit was het eerste dat ik probeerde en het ging meteen mis — er mogen geen fouten in zitten, alles moet gewoon werken").** Ná de bug-fix hierboven: systematische end-to-end-test van alles wat een volwassene moet kunnen om huiswerk voor te bereiden. Test-matrix (elke rij = volledige keten op productie, live doorklikken, niet alleen code-trace):
  1. **Ouder → kind**: koppelcode maken → kind claimt code (cross-device) → ouder opent klaarzet-modus → les klaarzetten (💛) → verschijnt bij kind onder "Speciaal voor jou klaargezet" met bron "thuis" → kind start + rondt af → afvink-status terug bij ouder → les weghalen (×) → "meer klaarzetten".
  2. **Leerkracht → leerling**: zelfde keten via /leerkracht + LeraarKlaarzet (koppelcode met teacher_user_id, bron "juf") — incl. de 15-aug-RLS-les (rol-specifieke policies).
  3. **Kind-kant**: blok toont beide bronnen correct gebundeld (`voor_jou_klaargezet`), afvinken schrijft naar de juiste tabel, geen verwarring met de algoritmische "Dit staat voor jou klaar"-lijst in MijnPagina.
  4. **Klas-varianten**: Takenlijst (deelcode TAAK24) + Toets-deelcode — klaarzetten, leerling opent, resultaat terug.
  5. **Randgevallen**: verkeerde/verlopen code, kind zonder account, tweede kind koppelen, klaarzet-modus verlaten/annuleren, dubbel klaarzetten (unique-constraint), uitgelogd raken middenin.
  Werkwijze: Playwright op productie met échte test-accounts (Google-login-stap = Mark even meeklikken of test-sessie), per rij ✅/🔴 loggen; élke 🔴 direct fixen in dezelfde sessie en de rij opnieuw draaien. Klaar = alle rijen groen + kort matrix-rapport aan Mark. Dit is de "huiswerk voorbereiden"-belofte richting Froukje/scholen — moet vlekkeloos zijn vóór we het actief aanbieden.

- [ ] **🔴 PO-content-gaten groep 3-5 vullen (Mark 12 aug: "bouw waar gaten zitten"; curriculum-kaart live in v272).** Gemeten stand (pathManifest): groep 3 = alléén rekenen (2 paden); groep 4 = rekenen 3 + spelling 1 + natuur 1 (taal 0! begrijpend-lezen 0!); groep 5 redelijk maar dun (spelling 3, BL 3, aardrijkskunde 1). De "🔨 hieraan bouwen we"-rijen op /mijn tonen deze gaten nu eerlijk + 🔔-interesse-knop meet de vraag (learn_path_waitlist, event vak_interesse). **Bouwvolgorde (volgende sessie(s), agents):** ① taal groep 3-4 ("leren lezen"/woordenschat/zinnen, 2 paden) · ② begrijpend-lezen groep 4 (korte teksten, 1-2 paden) · ③ spelling groep 3 · ④ verkeer groep 4-6 (bestaat 1 pad g5-8 — level verbreden ná didactische check?) · ⑤ natuur groep 3-4. Werkwijze: bestaand PO-pad als voorbeeld (bv. spelling/tafelsPo), 15-min-delen, uitlegPad 3 niveaus, wrongHints-regels, geen verzonnen examens. Interesse-teller in dagrapport meenemen.
...
- [x] **Leer→beloning-lus v1 = 🏆 DIPLOMA-KAST (Mark koos 12 aug: "tuin-les + mini-diploma's als PDF, ook voor ouders")** ✅ v271: `src/shared/ui/DiplomaKast.jsx` — beste score per onderwerp uit leaderboard = mini-diploma (%, datum, "beste van N pogingen"); printbaar via /diploma (nu mét ?score= en ?datum= op het certificaat). Staat op /mijn (kind), in het thuis-overzicht (ouder, uit childScores) én als 🏆-prijzenkast-overlay in het park (HUD-knop + "Verdien je volgende diploma →"-knop naar leren). Tuin-les toegepast: groei zichtbaar (hogere score = diploma groeit mee), nooit straf. **Open v2-ideeën:** diploma-moment vieren op het klaar-scherm (confetti + directe print-knop) · nieuwe-diploma-regel in de maandag-weekmail · 3D-prijzenmuur-object ín het park zelf · mastery-goud ("beheerst") als apart diploma-type.
...
- [ ] **Kwartiercheck v1.1 — restpunten uit Fable-review 28 jul** (v111 fixte de 5 bugs: adaptieve niveau-selectie, dode mail-link, 2 foute antwoorden, dubbele optie; + spam-guard op de API; + stadspas-claims eerlijk). ✅ 28/7 v114 EXTRA (Mark-keuze "wekelijks"): vervolgreeks gebouwd — na de resultaat-mail elke maandag 1 week uit het plan (max 3 mails), zwakste onderwerpen eerst, toets-countdown + zelfprint-tips per onderwerp; lift op de dagelijkse cron (maandag-blok); afmeld-token nu écht werkend (oude ?email=-link was dood) en afmelden stopt beide tabellen. EERSTE ECHTE RUN: maandag 3 aug 06:30 — check het dagrapport op "🧭 kwartiercheck-week". Later (jan 2027, eindsprint): opt-in dagelijkse oefenmail laatste 4-6 wkn vóór de toets. Nog open, op volgorde van impact:
  ...
  5. **CONCEPTEN centraliseren** (staat dubbel in conceptMapping.js + api/kwartiercheck-mail.js — drift-risico, B2 moest al op 2 plekken).
  6. **emailVanIntro benutten** (App.jsx ~r2052 hardcoded null) — prefill uit bestaande GratisLesmateriaal-inschrijving.
  7. Didactiek-na-lopen niveau-3-vragen (ongebruikt in v1 maar tafels-3a "7 broden in 9 soorten" is dubbelzinnig; hoofdgedachte-2b toetst grammatica onder leesconcept).
  8. Design-system: KwartiercheckPage volledig inline-stijlen ipv src/shared/ui/.

- [ ] **wrongHints-kwaliteitssprint — ECHTE cijfers na linter-fix 14 jul** — de parser van `lint-wronghints.mjs` sloot éénregelige arrays niet (gefixt 14 jul) waardoor oude tellingen onzin waren. Werkelijke stand: **158× pattern-A** (alle foute hints identiek → eliminatie-lek), **144× pattern-D** (correct antwoord letterlijk in hint — deels false positives zoals "wat is de stam van krijgen?"), **72× "Klopt"-hint op foute positie** (ernstigst: kind kiest fout, leest "Klopt"; maar let op false positives als "Klopt zeker niet"). Aanpak: géén bulk-rewrite (didactische schade); gefaseerd per bestand met menselijke blik, start bij Cito-kern (doorstroomtoetsRekenenG8 46, oppervlakteOmtrekPo 30, delenPo 26). Voorbeeld-patroon gefixt 14 jul in alfabetWoordenboekPo (2×): herschrijf naar vergelijkende denkprikkel in vraagvorm. Tools: `lint-wronghints-eliminatie.mjs` (pattern A+D, antwoord-bewust) + `lint-wronghints-klopt.mjs`. ✓15/7 **top-6 files schoon** (77→71 files met hits): volgordeBewerkingen (8, `2d46f5f`), informaticaHavoVwo (11, `9284118`+`0e19a7a`), bedrijfseconomieHavoVwo (6+4, `a5f3ee0`), balansBeco (5+2, `8268e7e`), onderwijsNiveausVmboHavoVwo (5, `8268e7e`), spreekwoordenUitdrukkingenPo (4+kale, `1173f2f`) — alle herschreven naar per-misvatting-denkprikkels, kale "Niet."-hints → null. Resterend: 71 files met ≤4 hits (grootste: eetcultuur/engels-schrijven/maatschappijleer 4) — zelfde werkwijze, per file met menselijke blik. ✓22/7 **top-6 files schoon** (71→65): eetcultuurNederlandPo (4), engelsSchrijvenSprekenHavoVwo (4, + kale `nogSimpeler:"A."` gefixt), maatschappijleerHavoVwo (4), wereldoorlog1Geschiedenis (3), ruimtevaartPo (3), pubertijdGroeiPo (3) — 21 hints herschreven naar per-misvatting-denkprikkels in vraagvorm. Resterend: 65 files met ≤3 hits. ✓22/7 **batch 2 schoon** (65→60): beroepenWerkPo, brugklasOrientatie, dichtenPoezieRijmenPo, industrieleRevolutiePo, oudheidEgyptiansGriekenPo — 15 hints herschreven. Resterend: 60 files. ✓22/7 **batch 3 schoon** (60→54): aardobservatieHavoVwo, cseSchrijfvaardigheidEngels, gezondeVoedingPo, latijnVwo, maatschappijwetenschappenHavoVwo, nederlandsCseHavoVwo — 18 hints herschreven. Resterend: 54 files (≤3 hits elk). ✓22/7 **batch 4 schoon** (54→48): bbpConjunctuurEconomie, bekendeBoekenLiteratuurPo, waterkringloopPo, stoffenMengselsScheikunde, schattenAfronden, mensenrechtenMaatschappijleer — 14 hints herschreven. Resterend: 48 files (≤2 hits elk). ✓22/7 **EINDBATCH via 3 parallelle agents — pattern A = 0** (48→0 files, 59 hits herschreven, steekproef gecontroleerd: per-misvatting-denkprikkels in vraagvorm zonder verklap). 🏁 **wrongHints-eliminatie-sprint AFGEROND**: totaal 22 jul 71→0 files (127 hints herschreven in 5 batches, v76-v80). Nog open uit deze sprint: pattern-D check (antwoord letterlijk in hint, deels false positives) + de 450 kale letter-niveaus (aparte taak hieronder).
- [ ] **NIEUW ontdekt 22/7 — 450× kale letter-niveaus** `basis/simpeler/nogSimpeler: "A."` (hele waarde = alleen de letter) in tientallen files (aardobservatie, aiMachineLearning, e.v.a.). De eerdere strip-scripts (B0.5/B2.1) haalden alleen trailing " A." weg, niet waarden die geheel uit een letter bestaan. Deze niveaus zijn zinloos ná shuffle én bieden nul uitleg op het laagste niveau. Niet bulk-herschrijfbaar (per concept een echte mini-uitleg nodig); zelfde gefaseerde werkwijze als wrongHints-sprint. Detectie: `Select-String -Pattern '(basis|simpeler|nogSimpeler): "[A-D]\."'`.
...
## Sessie-log
- **6 sep 2026 (v601):** Mark's WhatsApp-plan aan Brian (14:52: "loopbrug tussen de bergen, bots erover, sneeuw, kabelbaan omhoog, slee omlaag") → **bergen-plan**: twee decorbergen naast de vulkaan zijn nu écht terrein (eilandVorm BERGEN + EchteBergen.jsx), sneeuwgrens 50 m (ook op de vulkaan, niet bij de krater), **Hangbrug** (50 m vulkaan↔Oostberg, leuning = muur, 2 wandelende blok-maatjes), **Kabelbaan** (dal→bergstation, instappen/uitstappen via teleportRef, rit-camera), **Sleebaan** (ijsgeul met 3 bochten, tot 13 m/s), 4 nieuwe leermomenten met vraag-per-plek, deel-links ?scene=kabelbaan/slee/hangbrug. Skiën bewust niet. Build groen, Playwright-ritten getest (les: rit-camera boven het terrein klemmen; camera-yaw op hellingen aan de dal-kant). Open: framerate slee-rit op echte telefoon, pad naar het dalstation, rommel-geluid.
- **6 sep 2026 (v598):** Mark (voor het slapen): "maak ook een vulkaan-leerpad op groep 6-8-niveau" → `vulkanen-po` (5 stappen, 21 vragen, ~12 min, uitlegPad op 5 kernvragen, Saba/Mount Scenery erin); park-leermoment 'vulkaan' wijst nu eerst naar dit pad, platentektoniek (klas 2-3) als vervolg; 15-min-gate + build groen.
- **6 sep 2026 (v597):** Mark: "Brian's vulkaan in mijn park, één berg vervangen, past bij aardrijkskunde" → **BuitenVulkaan** (Mayon-kegel 1:1 uit eiland.html op bergformaat, beklimbaar via buitenHoogte, lavameer-shader, rookpluim, uitbarsting elke 4 min zonder dood), poort + leerbord "Aardlagen & vulkanen" aan de voet (PoortWatcher VASTE_POORTEN), deel-link `/dierentuin?scene=vulkaan`; build groen, dev-screenshots OK. Open: rommel-geluid, gids-zin bij uitbarsting, PO-niveau vulkaan-pad (huidig pad = klas 2-3).
- **1 sep 2026 (v529-531):** Fable-naloop compleet (build+250 tests groen; koppeling-herstel v528 **live E2E bewezen** met 2 accounts: verse code → nieuw toestel → child_user_id verhangt automatisch → uitslag op ouder-dashboard; v527 cito-rij geverifieerd) · dode Translate-balk verwijderd uit index.html (Google-widget bestaat niet meer; v529) · misleidende "Bonus"-tegel weg van CitoPage (riep simulatie aan; v530) · mail 5d her-gecontroleerd (niets gemist; Steffy/VoorleesExpress-Steenbergen bedankt) · KINDERHULP2027 → ongelimiteerd (Mark-go) · **idee #90 gebouwd: /voorlezen.html** (buildVoorlezen.mjs in prebuild, telt 81 teksten/267 vragen uit Leesladder-data; FAQPage+LearningResource JSON-LD, sitemap+llms.txt, plakklaar nieuwsbrief-blokje; doorkliks = bron pagina:voorlezen; v531) · dagrapport + PDF (óók per Resend-mail naar telefoon — SendUserFile bestaat niet in deze omgeving).
- **28 aug 2026 (v490-495):** Froukje-sessie: ouder-route hard getest → 3 fixes (login-terug naar /ouder via lk_login_terug, rol "ouder" bij login op /ouder (upsert), code-balk + login-muur uitleg; v490-491) · naam-wis-race bij koude start + "Hoi !" gefixt (v492) · leerkracht-route getest + juf-zonder-account: apparaat-waarschuwing + 💾 kopie bewaren/terugzetten als bestand (v493) · 🔴 chunk-404 een jáár gecached door immutable-header → recovery heelt nu met cache:'reload' (v494) · **Familie-versie-ladder gestart (Mark-go): Redactiebladen versies A-C gratis + D-E Familie/bèta-gratis, 40 nieuwe nagerekende sommen, versie-kiezer + upsell na C (v495) + dictees 3× 💛 werkwoordspelling + tafelbladen 💛 gaten-sommen/grote getallen (v496). VOLGORDE-AFSPRAAK Mark 28 aug: (1) ✅ ladder dictees+tafels → (2) Pro-klassenset: dezelfde 💛-versies als klassenset met antwoordblad voor de juf → (3) oefenboekje-op-maat als Familie-kroonjuweel (prototype /oefenboekje bestaat). Redactiebladen F t/m J + werkboek/brugklas-ladder = doorlopend contentwerk.** Ouder/kind-lijn end-to-end groen (klaarzetten beide kanten, voortgang, afvinken, privacy, AVG-delete).
- **12 aug 2026 (v276):** Mark: "en voor de studenten" → **/mijn-vakkenkaart werkt nu ook voor klas 1-6** (VO): `parseNiveau` (groep óf klas uit profiel-level), `padBijKlas` (vangt klas1-2/klas2-3-vmbo-vwo/havo4-5-vwo/havo-vwo-4-5/vmbo-gt-4/vwo), `VAKKEN_PER_KLAS`+`VAK_INFO_KLAS`+`KLAS_NOTITIE` in vakkenPerGroep.js ("taal"→"Nederlands" op VO); alle teksten niveau-bewust ("Wat moet ik kennen in klas 2?", doel "Slagen voor je eindexamen" bij klas ≥4). Dekking gecheckt: **elke klas 1-6 heeft voor élk kaart-vak ≥1 pad** (198 VO-paden) — geen bouwtegels nodig, mechaniek blijft als vangnet. Geen nieuwe content gebouwd (klas 1-3-uitbreiding blijft bevroren per roadmap). Build groen.
- **12 aug 2026 (v275):** vervolg "hieraan bouwen we": **álle groepen 3-8 nu volledig gedekt** — 3 laatste paden: `taal-leren-lezen-g3` (klanken, rijmen, hakken/plakken, eerste woordjes+zinnetje; superkort-leesbaar voor 6-7 jr), `spelling-eerste-woorden-g3` (luisterwoorden, lange klanken aa/oo/ee/uu, oe/ie/eu/ui, sch-/-ng) en `geschiedenis-vroeger-en-nu-g5` (vroeger/nu, tijdbalk+eeuw, leven ~1925, ridders/kastelen). Zelftest 60 checks 0 fouten, 15-min-gate OK, build groen.
- **12 aug 2026 (v274):** Mark: "alles met 'hieraan bouwen we' oppakken" → **groep 4 compleet op /mijn**: 3 nieuwe paden `taal-woorden-zinnen-g4` (alfabet/meervoud/verkleinwoorden/tegenstellingen/zinnen), `korte-teksten-snappen-g4` (tekstlengte-ladder: 1 zin → 6-zinnen-tekstje, wie/wat/waar/volgorde/want-reden) en `verkeer-veilig-op-straat-g4` (oversteken/stoep-fietspad/lichten/fiets/borden, feiten RVV-gecheckt) — alle 6 groep-4-vakken hebben nu oefenstof. **Bijvangst-bugfix buildPathManifest.mjs**: .js-paden met .jsx-import (RekenOefenRonde, sinds 11 aug) én topografie-paden (import.meta.env) vielen stilletjes uit het manifest — 8 paden weg, nu esbuild-fallback + env-define; `continenten-wereld-po` daarmee hersteld (was al vóór deze sessie uit het gecommitte manifest verdwenen). 15-min-baseline bewust herijkt (delen-po 25m + continenten-wereld-po 25m = bestaand werk; TODO: inkorten/splitsen). Zelftest: 61 checks gevalideerd (answer/wrongHints/uitlegPad) 0 fouten, wronghints-lint schoon, build groen.
- **10 aug 2026 (v231-239):** dagrapport + GSC-Reviewfragmenten-fix zelfde ochtend (v231) · partner-welkom-banner idee #22 gebouwd op Mark-akkoord (Spark Fest-groet, 24 partner-namen, bulk-variant, ouder-voorrang, OOIEVAAR neutraal; v232) · PRIJSPLAN §6 toegangs-keten + gezinscode · tier-sprint T1-T7 AF (ProPage herbouwd naar besloten model, abonnement-stippen, weekmail-label, TopografieCheck-gratis, FamilieHub goud; T6 bewust geskipt: geen prijs-druk in kind-UI; v233-236) · examen-sprint: E0 AF (bio-2025-T1 → 12 via rechtenvrije Wikimedia-beelden), E1 deels (bio-2023-T1 6→9: ethogram-tabel, preening-fix, oog+tand), E5 teller-knop + prebuild-script, E6 gecheckt (v237-239) · bijvangst: ExamensPage hooks-order-bug gefixt, beeldOntbreekt-vangnet, wees-dev-server op 5199 opgeruimd.

Eén regel per sessie. Datum + wat gedaan + commit-hash van laatste push.

- 2026-07-29 — **Push-keten geverifieerd + Deel-actie 2027 in mails (v118, commit 1066f9b).** Push: Vercel-env ok (VAPID + SERVICE_ROLE, "lege" pull = sensitive-vars-artefact; live bundle bevat key), Mark = eerste abonnee (Chrome-voor-Claude, permission via CDP), edge function test-invoke `sent:1` (echte melding afgeleverd), pg_cron `leerkwartier-daily-push` actief 15:00 UTC. Referral: nieuwe service-only RPC `vriendencode_voor_email` (migratie live), weekmail heeft nu vast Deel-actie-blok mét persoonlijke ?vriend=CODE-link (fallback generiek), Kwartiercheck-mail kreeg compact Deel-actie-blok → ouder-dashboard. Lokale mock-test 4/4 groen. Mark's code = 6DAAC3. NB: kaartje na kind-quiz bewust NIET gedaan (STOPLIST — referral alleen op ouder-oppervlakken).
- 2026-07-28 — **v106-v109: voor-organisaties.html + llms.txt + C1/C2/C3** (laatste push d0ebfae). v106: `voor-organisaties.html` LIVE (SEO-pagina kansarme gezinnen/Leergeld/stadspassen + Schema.org) + `llms.txt` AI-directive. C1 (v107): `GratisLesmateriaal` op AllDone-scherm leerpad-speler + CitoPage had het al. C2 (v108): Noord-ster-metric (terugkerende apparaten 7d, SQL in CLAUDE.md, nu=7) + privacy.html `lk_uid` eerlijk vermeld. C3 (v109): Gratis Kwartiercheck LIVE — 12 concepten, adaptief max 2 vragen/concept, oordeel beheerst/bijna/nogniet, Resend-scorekaart+weekschema, Supabase `kwartiercheck_results` tabel, `public/kwartiercheck.html` + `?go=kwartiercheck` deeplink. **Volgende sessie (Fable 5):** Kwartiercheck testen (mail check), promoten via social+flyer-URL, wrongHints pattern-D + kale letter-niveaus.
- 2026-07-15 (2e, RC-autonoom) — **A8.3 kwartier-streak+dagdoel naar Supabase** (5 kolommen op profiles, merge-functie +6 tests, sync in useAuth, commit 0779238) + **wrongHints-sprint top-3 files schoon** (volgordeBewerkingen/informatica/bedrijfseconomie, 25 lekken → per-misvatting-denkprikkels, 77→74 files, laatste push a5f3ee0). Alles getest + gebuild per stap.
- 2026-07-15 — **A8.2 batch 2: 4 begrijpend-lezen-paden** (feiten-details / oorzaak-gevolg / alinea-tussenkopjes / nieuwsbericht-5W+H, 67 checks, 4 parallelle agents) + **trio-uitbreiding uit working tree gered** (+1623 regels taal/rekenen/studievaardigheden, eerdere sessie nooit gecommit) + buddy-TTS-fix (emoji's/"vs" niet meer voorgelezen, shared schoonVoorSpraak). 219 tests + build + linters groen; baseline herijkt. Laatste push 388e2d7 (+ backlog-commit).
...
- 2026-09-22 (Fable 5.1) — v681 Charley-dagbundel 120→20 · v682 prijslijst gelijk (partner €34,50 in config) + geen concurrenten bij naam · v683 Pro geschrapt, school op aanvraag · v684 **/klas** klas-ingang zonder rolkeuze + klasgolf-radar (docs/sql/klasgolf.sql) · docs/sql/noordster.sql + golven.sql aangelegd · verdienmodel-review §10 · flyer-generator `<strong>`-bug gefixt · cdp-eval.ps1 hersteld + gmail-antwoord.ps1.

- **2 sep 2026 (Fable) — Park samenhang-sprint 1 (v556):** 3-perspectief-review (leraar/kind/developer) → `docs/plannen-park/PARK-SAMENHANG-PLAN-2026-09-02.md` ("elke plek stelt één vraag"). Gebouwd: vraag per leerobject (parkLeermomenten.js), poort = uitnodigings-kaart i.p.v. auto-teleport, gids/poorten óók aan in bouw-modus, gids-toast met vraag + "▶ Naar de les", maatje-wolkje over de plek waar je staat (tikbaar), AI-chat kent plek + alle leerobjecten, takenbord "🗺️ Taken x/y" (parkTaken.js eindelijk gerenderd). OPEN sprint 2: niveau-mismatches vormen/boom/achtbaan (→ PO-paden), één kiezer volgendeLeerstap(), poorten op de groene route, PARK_LEERMOMENTEN als enige bron (band/vak), park_les_afgemaakt-event.

- **2 sep 2026 (Fable) — Park samenhang-sprint 2 (v557):** vormen+zwembad → PO-pad `maten-omtrek-oppervlakte-po` (brugklas-pad als `leerpadIdVo` + "🎓 Brugklas-versie"-knop; gouden-vormen-teller telt beide paden); band/vak in PARK_LEERMOMENTEN (`TAAK_VOLGORDE`, parkTaken afgeleid; weegschaal→gr7, telraam→gr6, boom+zweefmolen→brugklas); één kiezer `volgendeLeerstap.js` (klaargezet→herhalen→zwak) voor leer-invite/maatje/chat/wandeling met reden-zinnen; kleine poorten vóór de 6 bord-objecten op de groene route (`POORT_KLEIN_ASSETS`); event `park_les_terug`. Les: dev-test = eerst SW + caches wissen (stale modules). OPEN sprint 3: wandeling = het kwartier (3 stops→3 vragen→dagdoel), spawn bij open taak, wandelvoortgang naar Supabase; bezoeker-dialoog uitzetten.

- **2 sep 2026 (Fable) — Park samenhang-sprint 3 (v558):** wandeling = het kwartier: route af → `WANDEL_REWARD` 15 munten + badge "stop x van 3 gevonden" in het paneel (takenbord-opening telt níét als stop); wandeling in Supabase `owned.wandeling` + herstel op ander toestel (`herstelWandeling`); gids-zin bij binnenkomst met open wandeling ("loop naar … + vraag"); bezoeker-dialoog UIT (`BEZOEKER_PRAATJE_AAN=false`). Bewust NIET: auto-spawn bij open taak (scene mount vóór park-load → teleport-risico). Live E2E groen (kies route → 3 stops → viering → munten 95→110 → cloud-rij). Samenhang-plan sprint 1-3 AF; meetlat over 30 dgn in dagrapport.

- **2 sep 2026 (Fable) — F15 double opt-in (v559):** zie F15-regel in de Fable-sprint. Nieuwe hosting-regel: nooit méér dan 21 bestanden in `api/` (Hobby-limiet 12 bundels); nieuwe endpoints in `api/_lib/` + rewrite.

- **3 sep 2026 (Fable) — Kliktocht ná v555-566 (2 agents, code-trace):** 0×P0. Gefixt v567: 7 terugknop-routes (vak-tegel→hub→terug, Results→les→terug, voortgang/ouder-dashboard/maatje terug naar vorige pagina via `terugNaarVorige`, park→hub, 🔁-wissel wist kind-wissel-bewaking); mailflow: bevestig-link hoofdletter-ongevoelig + e-mail lowercased bij insert, plan `weekpakket` in weekmail-selectie, partner-meelezer krijgt eigen mail mét afmeld-link (`/api/unsubscribe?partner=`); TeacherHome 🏠; Cito-sweep-restjes ("voorheen Cito-eindtoets", Eindopdracht, Voorbeelden uit de toets, Cito-examenboekje, "Alle onderdelen"). Open P2 (dood gewicht props: onPlayObliterator/prefilterVak/parkNaam e.a.) → later opruimen.
