...
- 2026-06-07 — **Obliterator engagement (15-agent-analyse + kritische review).** Quick wins LIVE (commit 88d6235): Fase 1 SCORE_MUL 10→11 (+10%, geen DB-migratie); Fase 2 `laadTopScoresWeek()` + Dag/Week/Aller-tijden segment-control (week default) + lokale NL-daggrens i.p.v. UTC-bug; Fase 3 rang-systeem `src/games/obliterator/rank.js` op hoogst-behaald level (Bronze→Legendary, client-side, cosmetisch, badge naast score + eigen record). **🔴 MARK-AKKOORD NODIG (security must-fix vóór score-afgeleide features echt eerlijk zijn — schema, niet autonoom):** (1) RLS dichttimmeren `obliterator_scores` (anon INSERT = score-injectie → nep-Legendary; CHECK score>=0 + plafond, anon van publiek bord weren); (2) naam-identiteit harden (user_id-dedupe + player_name = profiles.display_name server-side; profanity/URL-filter tegen pesten); (3) offline-flush stuurt created_at niet mee + geen dedupe-key. **Medium (later):** skins UITBREIDEN (12 bestaan al constants.js:220 — onderste 2-3 laagdrempelig via missie/streak), persoonlijk-record-balk bij game-over, game-events voor KPI. **Niet doen (Critical Reviewer):** ghost/nep-rijen, FOMO-streak-copy, seizoen-tabellen/vriendcodes = dark-pattern/scope-creep.
